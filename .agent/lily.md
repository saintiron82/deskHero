...
진행_상황_자동_리포트:
  ...
  품질_지표:
   ...
   4. **🛑 사용자 승인 확인 절차 (Pre-Implementation Check)**:
      - **원칙**: Jina 또는 사용자로부터 "구현 승인"을 명시적으로 확인하기 전에는 코드를 작성하지 않음.
      - **대기 상태**: 계획(Plan) 수립 후에는 반드시 `notify_user` 등을 통해 사용자의 재가를 기다림.
      - **예외**: 사용자가 명시적으로 "바로 진행해"라고 지시한 경우.
```
